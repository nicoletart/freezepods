  ...
  if (!navigator.bluetooth) {
    ...
    navigator.bluetooth
      ...
      .then((device) => {
        ...
        addLog(
          "Connecting to GATT server (name: <font color='blue'>" +
            device.name +
      ...
      .then((service) => {
        ...
        service
          .getCharacteristic(microbitUuid.buttonAState[0])
          .then((buttonAChar) => {
            addLog("<font color='green'>OK</font>", true);
            ...
            return buttonAChar
              ...
              .then((_) => {
                addLog("<font color='green'>OK</font>", true);
                buttonAChar.addEventListener(
                  "characteristicvaluechanged",
                  buttonAChanged
                );
                addLog("Getting Button B state characteristic... ", false);
                service
                  .getCharacteristic(microbitUuid.buttonBState[0])
                  .then((buttonBChar) => {
                    addLog("<font color='green'>OK</font>", true);
                    buttonBStateCharacteristic = buttonBChar;
                    console.log("Starting button B notifications... ");
                    return buttonBChar
                      .startNotifications()
                      .then((_) => {
                        addLog("<font color='green'>OK</font>", true);
                        buttonBChar.addEventListener(
                          "characteristicvaluechanged",
                          buttonBChanged
                        );
                      })
        ...
        service
          .getCharacteristic(microbitUuid.pinData[0])
          .then((pinData) => {
            //addLog("<font color='green'>OK</font>", true);
            pinDataCharacteristic = pinData;
            console.log("Starting pin data notifications... ");
            return pinData
              .startNotifications()
              .then((_) => {
                // addLog("<font color='green'>OK</font>", true);
                pinData.addEventListener(
